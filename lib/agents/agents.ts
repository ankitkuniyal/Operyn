...
import { createDraft, fetchUnreadEmails, markAsRead } from "./gmail";
import {
  CalendarEvent,
  createCalendarEvent,
  fetchUpcomingEvents,
...
import { anaylzeWithAI } from "./process-email";
    ...
    for (const email of emails) {
      try {
        ...
        for (const item of analysis.actionItems) {
          ...
          if (item.dueDate) {
            const tempDate = new Date(item.dueDate);
        ...
        totalTasksCreated += emailTasksCreated;
